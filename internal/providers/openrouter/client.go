...
func (c *Client) Generate(ctx context.Context, req domain.GenerateRequest) (*domain.GenerateResponse, error) {
	if c.apiKey == "" {
		return nil, fmt.Errorf("openrouter api_key is empty")
	}
	...
	httpReq.Header.Set("HTTP-Referer", "https://prompt-sensei.local")
	httpReq.Header.Set("X-Title", "PromptSensei")
	...
	logging.Debug("openrouter response status", "status_code", resp.StatusCode, "status", resp.Status)
